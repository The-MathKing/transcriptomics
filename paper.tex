...
\section{Results}
\label{sec:results}
Our results (Figure \ref{fig:rel}) show that DestVI has an In-Distribution Expected Calibration Error (ECE) of 0.226. This baseline indicates that out-of-the-box variational models exhibit some degree of miscalibration. 

Under cross-platform shift (80\% capture efficiency reduction), the calibration degraded significantly, with ECE increasing to 0.249. By optimizing temperature scaling on the shifted predictions, we restored calibration and reduced the ECE to 0.086.
