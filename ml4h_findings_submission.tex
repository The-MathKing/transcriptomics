...
To simulate synthetic capture-efficiency shift motivated by cross-platform variability, we synthetically downsampled the read counts of the held-out test split via a binomial dropout process \citep{townes2019feature}.
...
As a structural intermediate, we also evaluate Beta[a=b] calibration \citep{kull2017beta}, a two-parameter parametric mapping applied via logistic regression on the log-odds of the predicted confidences. This serves to directly test the ``single-parameter versus non-parametric'' flexibility spectrum.
...
While synthetic shifts isolate technical noise, real-world deployments involve biological cross-platform shifts. We found spatial models are highly sensitive to biological misspecification (uncalibrated ECE $>0.385$). Furthermore, blindly applying default synthetic recalibration ($T=2.0$) to an already-harmonized, well-calibrated DropViz-to-Slide-seqV2 mapping severely degraded calibration (ECE $0.0642 \rightarrow 0.1646$), demonstrating that blindly applying default recalibration harms an already-calibrated model.
...
We note several limitations. First, while our invariant construction cleanly isolates the conditional vulnerability of Isotonic Regression, our core claim—that fine-grained spatial taxonomic mappings fundamentally restrict confidence density support and precipitate relative shift degradation—remains strictly untested outside of synthetic data. To ensure feasibility, models were trained for up to 200 epochs using early stopping on 50\% of available pseudo-spots. Furthermore, our synthetic evaluations rely on binomial dropout and Poisson noise on mouse cortex data, though offset by qualitative real-tissue checks. Future research should investigate inherently risk-calibrated architectures.
...
To compute the theoretical ECE noise floor for a perfectly calibrated model, we simulated 5,000 empirical datasets. For each simulation, we sampled ground-truth test proportions from a uniform Dirichlet distribution across $k$ classes. To simulate $N=500$ perfectly calibrated spatial spots, we defined the ground-truth label as the argmax of these spot-level true proportions, matching our estimand $Y_{\text{dom}}$. We then computed the maximum probability (confidence) and argmax class (prediction) for each spot, and evaluated the fixed-bin Expected Calibration Error (10 bins) against the generated labels. Because the predictions are perfectly calibrated by definition, any non-zero ECE arises strictly from finite-sample binning noise. This yielded a mean expected noise floor of $0.021 \pm 0.010$ for $k=23$, and $0.041 \pm 0.013$ for $k=4$.
