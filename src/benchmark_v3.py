...
def get_calibration_stats(true_p, pred_p, temp=1.0, iso_reg=None):
    ...
    return conf, acc, ece, brier, comp_rmse, cal_pred_p
...
def optimize_temperature(true_p, pred_p):
    ...
    for t in np.linspace(0.5, 3.0, 50):
        _, _, ece, _, _, _ = get_calibration_stats(true_p, pred_p, temp=t)
        if ece < best_ece:
            best_ece = ece
            best_t = t
...
def run_kfold_pipeline(adata_sc, fractions, noise_levels, epochs, n_splits=10):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(np.zeros(len(y_strat)), y_strat)):
        ...
        for frac in fractions:
            ...
            _, acc_raw, ece_raw, brier_raw, comp_rmse_raw, _ = get_calibration_stats(true_props_test, pred_props_test)
            
            # Evaluate Clean Calibrators
            _, _, ece_temp, brier_temp, comp_rmse_temp, _ = get_calibration_stats(true_props_test, pred_props_test, temp=best_t)
            _, _, ece_iso, brier_iso, comp_rmse_iso, _ = get_calibration_stats(true_props_test, pred_props_test, iso_reg=iso)
            
            # Evaluate Shifted Calibrators
            _, _, ece_temp_shifted, _, comp_rmse_temp_shifted, _ = get_calibration_stats(true_props_test, pred_props_test, temp=best_t_shifted)
            _, _, ece_iso_shifted, _, comp_rmse_iso_shifted, _ = get_calibration_stats(true_props_test, pred_props_test, iso_reg=iso_shifted)
            ...
            seed_results["fractions"][frac] = {
                ...
                "ece_temp": ece_temp,
                "comp_rmse_temp": comp_rmse_temp,
                "ece_iso": ece_iso,
                "comp_rmse_iso": comp_rmse_iso,
                "ece_temp_shifted": ece_temp_shifted,
                "comp_rmse_temp_shifted": comp_rmse_temp_shifted,
                "ece_iso_shifted": ece_iso_shifted,
                "comp_rmse_iso_shifted": comp_rmse_iso_shifted,
                "best_t": best_t,
                "best_t_shifted": best_t_shifted
            }
...
def main():
    ...
    for frac in fractions:
        ...
        final_output[f_str] = {
            ...
            "ece_iso_shifted": [r["fractions"][frac]["ece_iso_shifted"] for r in all_results],
            "ece_temp_shifted": [r["fractions"][frac]["ece_temp_shifted"] for r in all_results],
        }
    ...
    for nl in noise_levels:
        ...
        final_output[nl_str] = {
            ...
            "ece_iso": [r["noise_levels"][nl]["ece_iso"] for r in all_results],
            "ece_temp": [r["noise_levels"][nl]["ece_temp"] for r in all_results],
            "ece_iso_shifted": [r["noise_levels"][nl]["ece_iso_shifted"] for r in all_results],
            "ece_temp_shifted": [r["noise_levels"][nl]["ece_temp_shifted"] for r in all_results],
        }
