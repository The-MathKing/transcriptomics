...
def generate_pseudo_spots(adata_sc, all_cell_types, n_spots=2000, cells_per_spot=10, cell_type_col="cell_class", seed=42):
...
def get_calibration_stats(true_p, pred_p, temp=1.0, iso_reg=None):
    ...
    return conf, acc, ece, ece_adapt, brier, comp_rmse, high_conf_precision, cal_pred_p
...
def evaluate_bootstrapped_calibrators(true_props_cal, pred_props_cal, true_props_test, pred_props_test, n_bootstraps=100):
    ...
    ece_isos, ece_temps = [], []
    ece_isos_adapt, ece_temps_adapt = [], []
    hc_isos, hc_temps = [], []
    for _ in range(n_bootstraps):
        ...
        _, _, ece_t, ece_t_adapt, _, _, hc_t, _ = get_calibration_stats(true_props_test, pred_props_test, temp=best_t)
        ece_temps.append(ece_t)
        ece_temps_adapt.append(ece_t_adapt)
        if not np.isnan(hc_t): hc_temps.append(hc_t)
        ...
        if not np.isnan(hc_i): hc_isos.append(hc_i)
        
    return {
        "ece_temp_mean": float(np.mean(ece_temps)), "ece_temp_std": float(np.std(ece_temps)),
        "ece_temp_adapt_mean": float(np.mean(ece_temps_adapt)), "ece_temp_adapt_std": float(np.std(ece_temps_adapt)),
        "ece_iso_mean": float(np.mean(ece_isos)), "ece_iso_std": float(np.std(ece_isos)),
        "ece_iso_adapt_mean": float(np.mean(ece_isos_adapt)), "ece_iso_adapt_std": float(np.std(ece_isos_adapt)),
        "hc_prec_temp_mean": float(np.mean(hc_temps)) if hc_temps else float('nan'),
        "hc_prec_iso_mean": float(np.mean(hc_isos)) if hc_isos else float('nan')
    }
...
def main():
    ...
    out_name = "results_class_kfold.json" if not SMOKE_TEST else "results_smoketest_kfold.json"
